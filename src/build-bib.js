...
var getNamesWithInits = function(authors) {
  ...
  shortName += names[names.length - 1].replace(key, replaceChars[key]);
  return shortName;
};

var parseAuthors = function(authors) {
  return authors.split(' and ').map(function(name) {
...
function getJCTitle(entry) {
  ...
  if (entry.hasOwnProperty('booktitle')) return cleanTitle(entry['booktitle']);
  return '';
};

function getDOI(entry) {
  if (entry.hasOwnProperty('doi')) return entry['doi'];
  return '';
};

function getArxivId(entry) {
  if (entry.hasOwnProperty('eprint')) return entry['eprint'];
  return '';
};

var rawData = fs.readFileSync('./assets/kspubs.bib').toString();
var entries = bibtexParse.toJSON(rawData);
var bibEntries = entries.filter(function(entry) {
  var entryType = entry['entryType'] 
  return (
      entryType == 'Article' || entryType == 'InProceedings');
});
...
for (var i = 0; i < bibEntries.length; ++i) {
  ...
  jsonEntry = {
    ...
    'fullAuthors':
        fullAuthors.slice(0, -1).join(', ') + ' and ' + fullAuthors.slice(-1),
    'JCTitle': JCtitle,
...
fs.writeFile(
    './assets/pubs.json', JSON.stringify(jsonEntries, null, 2), function(err) {
